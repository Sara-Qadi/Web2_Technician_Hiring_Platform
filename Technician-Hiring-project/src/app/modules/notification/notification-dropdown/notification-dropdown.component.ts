...
export class NotificationDropdownComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.profileService.getUser().pipe(
      ...
      switchMap((user: any) => {
        if (!user?.user_id) return of(null);
        ...
        return this.notificationService.getNotifications(this.userId).pipe(
          catchError((err) => {
      ...
      // if user was null, data may be null
      if (!data) {
        this.notifications = [];
        return;
      }
    ...
    userIds.forEach((id) => {
      if (!this.userNames[id]) {
        this.profileService.getUserById(id).pipe(
          ...
          this.userNames[id] = user.user_name || `User #${id}`;

          if (user.role_id === 1) {
            this.userDetails[id] = {
              name: this.userNames[id],
              imageUrl: 'assets/person1.jpg',
            };
          } else {
